				  ...
				  dsFunctions *fn)
{
  ...
  for (int i=1; i<argc; i++) {
    ...
    if (strcmp(argv[i],"-framestep")==0) {
        char *endptr;
        long n = strtol(argv[++i],&endptr,10);
        if ( *endptr == 0 ) framestep = n;
    }
  }
...
aDrawEllipsoid(const float pos[3], const float R[12], const float sides[3])
{
    ...
    glShadeModel (GL_SMOOTH);
    setTransform (pos,R);
    glScaled(sides[0]/2, sides[1]/2, sides[2]/2);
    drawSphere();
    glPopMatrix();
    glDisable (GL_NORMALIZE);
...
{
    int i;
    float pos2[3],R2[12],sides2[3];
    for (i=0; i<3; i++) pos2[i]=(float)pos[i];
    for (i=0; i<12; i++) R2[i]=(float)R[i];
    for (i=0; i<3; i++) sides2[i]=(float)sides[i];
    aDrawEllipsoid (pos2,R2,sides2);
}
